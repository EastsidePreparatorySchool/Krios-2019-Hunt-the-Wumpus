...
public class LTDescr
{
    ...
    public void reset()
    {
        ...
        this.hasUpdateCallback = this.useEstimatedTime = this.useFrames = this.hasInitiliazed =
            this.onCompleteOnRepeat = this.destroyOnComplete =
 ...
 = child.gameObject.renderer.material.color; this.fromInternal.x = col.a; break; }}}
        ...
        this.easeInternal = () =>
        {
            ...
            if (this.spriteRen != null)
            {
                this.spriteRen.color = new Color(this.spriteRen.color.r, this.spriteRen.color.g, this.spriteRen.color.b,
                    val);
                alphaRecursiveSprite(this.trans, val);
            }
            else
    ...
    public LTDescr setGUIMove()
    {
        ...
        this.easeInternal = () =>
        {
            ...
            this._optional.ltRect.rect =
                new Rect(v.x, v.y, this._optional.ltRect.rect.width, this._optional.ltRect.rect.height);
        };
        return this;
    }
...
//		Debug.Log ("tween:" + this+ " dt:"+dt);
        if (this.delay <= 0f && directionLocal != 0f)
        {
            ...
            this.ratioPassed =
                (this.passed /
                 this.time); // need to clamp when finished so it will finish at the exact spot and not overshoot
            ...
            if (isTweenFinished)
            {
                ...
                isTweenFinished =
                    this.loopCount == 0 ||
                    this.loopType == LeanTweenType.once; // only return true if it is fully complete
